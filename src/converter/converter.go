...
func convertFile(request model.ConvertRequest) (HttpStatusCode int, e error) {
	...
	cmd, err := exec.Command(os.Getenv("BF_TOOLS_CONVERT_PATH"), "-overwrite", "-pyramid-resolutions", "6", "-pyramid-scale", "2", fijiRequest.OutputFile, request.OutputFile).CombinedOutput()
